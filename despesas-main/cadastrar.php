          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <select id="idLivro" name="idLivro" required>
                <option value="" <?= $idLivro === '' ? 'selected' : '' ?>>Selecione um livro</option>
                <?php foreach ($livros as $livro): ?>
                  <option value="<?= htmlspecialchars($livro[0]) ?>"
                    <?= ($idLivro === (string) $livro[0]) ? 'selected' : '' ?> >
            ...
            <div class="form-group">
              ...
              <select id="matricula" name="matricula" required>
                <option value="" <?= $matricula === '' ? 'selected' : '' ?>>Selecione um aluno</option>
                <?php foreach ($alunos as $aluno): ?>
                  <option value="<?= htmlspecialchars($aluno[1]) ?>"
                    <?= $matricula === $aluno[1] ? 'selected' : '' ?> >
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <input type="date" id="dataEmprestimo" name="dataEmprestimo"
                     value="<?= htmlspecialchars($dataEmprestimo) ?>" required>
            </div>
            <div class="form-group">
              <label for="dataDevolucao">Data de devolução *</label>
              <input type="date" id="dataDevolucao" name="dataDevolucao"
                     value="<?= htmlspecialchars($dataDevolucao) ?>" required>
            </div>
          </div>
